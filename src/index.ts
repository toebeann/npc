        ...
        listen: async (endpoint: string, options?: NpcListenOptions) => {
            ...
            return new Promise<NpcPublisher>((resolve, reject) => {
                server = createServer((socket) => {
                    sockets.add(socket);
                    socket
                        .once('close', () => sockets.delete(socket))
                        .once('error', () => sockets.delete(socket));
                })
                    .once('error', (error) => reject(error))
                    .listen(
                        {
            ...
            await Promise.all(
                [...sockets]
                    ...
                    .map(
                        (socket) =>
                            new Promise<void>((resolve) =>
                                socket.write(stringified, encoding, () =>
                                    resolve()
                                )
                            )
                    )
...
const getSubscriptionMap = () =>
    subscriptionMap ?? (subscriptionMap = new Map());
...
export function subscribe(
    ...
    const hasSeparateEventName = z
        .string()
        .safeParse(subscriberOrEventName).success;

    const eventName = hasSeparateEventName
        ? z.string().parse(subscriberOrEventName)
        : qualifiedEventNameSchema
              .parse(qualifiedEventNameOrEndpoint)
              .split('@')[0];
    const endpoint = hasSeparateEventName
        ...
        : qualifiedEventNameSchema
              ...
              .split('@')[1];
    const callback = (
        hasSeparateEventName ? subscriber : subscriberOrEventName
            ...
            socket: new Promise<Socket>((resolve, reject) => {
                try {
                    const socket = connect(
                        ...
                        .once('close', () =>
                            getSubscriptionMap().delete(endpoint)
                        )
                        .once('error', () =>
                            getSubscriptionMap().delete(endpoint)
                        );
...
export function unsubscribeAll(endpoint?: string, eventName?: string) {
    ...
    if (endpoint && eventName) {
        ...
        if (subscriptionMap.get(endpoint)?.subscriptions.size === 0) {
            subscriptionMap
                .get(endpoint)
                ?.socket.then((socket) => socket.destroy());
            subscriptionMap.delete(endpoint);
        }
    } else if (endpoint) {
        subscriptionMap
